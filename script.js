...
const renderCoinResults = (coins) => {
  ...
  coins.forEach((coin) => {
    ...
    card.addEventListener("click", () => addToInventory(coin));
...
const spinReels = async () => {
  ...
  if (doubleWinNext && payoutValue > 0) {
    credits += payoutValue;
    payoutValue *= 2;
    doubleWinNext = false;
  ...
  if (winBonusTurns > 0 && payoutValue > 0) {
  ...
  if (payoutValue > 0) {
    debt = Math.max(0, debt - payoutValue);
    updateDebt();
  }
